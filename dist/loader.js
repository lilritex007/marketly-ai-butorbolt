 ...
 * FINAL VERSION - No detection, direct bundle name
 */
...
(function() {
  ...
  const loadCSS = () => {
    ...
    const existingLink = document.querySelector('link[href*="index-C3I_80N7.css"]');
    if (existingLink) {
    ...
    link.rel = 'stylesheet';
    link.href = `${CDN_BASE}/assets/index-C3I_80N7.css`;
  ...
  // React bundle betöltése - DIRECT BUNDLE NAME (NO DETECTION)
  const loadReactApp = async () => {
    ...
    // ALWAYS use current build bundle directly - NO DETECTION!
    // Updated: 2025-01-27 - Bundle: index-Hbdtv5Wn.js
    const currentBundle = 'index-Hbdtv5Wn.js';
    const bundlePath = `/assets/${currentBundle}`;
    console.log('🔍 Using bundle DIRECTLY (no detection):', currentBundle);
