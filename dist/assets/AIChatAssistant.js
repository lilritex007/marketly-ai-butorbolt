...
`),s+='💬 Kérdezz bátran - pl. "modern kanapé 150 ezer alatt"');k([{id:C(),role:"assistant",content:s,timestamp:new Date,products:n.length>0?n:void 0}])},[r,$]);const _=()=>{var e;(e=D.current)==null||e.scrollIntoView({behavior:"smooth"})};h.useEffect(()=>{_()},[y]),h.useEffect(()=>{j&&z.current&&setTimeout(()=>{var e;return(e=z.current)==null?void 0:e.focus()},100)},[j]);const F=(e,a)=>{var n;const s=y.find(m=>m.id===e);s&&(xe(e,a,{query:s.userQuery||"",productCount:((n=s.products)==null?void 0:n.length)||0}),k(m=>m.map(o=>o.id===e?{...o,feedback:a?"positive":"negative"}:o)))},U=(e,a)=>{M&&M(e,a),S(!1)},H=async()=>{var m,o,x,f;if(!N.trim()||v)return;const e=N.trim();L(""),pe(e);const a=C();k(l=>[...l,{id:a,role:"user",content:e,timestamp:new Date}]),K(!0);let s=[],n=null;try{const l=await X(e,{limit:12});s=l.results||[],n=l.intent;const c=fe(),d=T(3),u=J(3),p=Z(),b=y.slice(-4).map(i=>`${i.role==="user"?"VÁSÁRLÓ":"TANÁCSADÓ"}: ${i.content.slice(0,150)}`).join(`
`),se=(r==null?void 0:r.categories.slice(0,10).map(i=>`• ${i.name}: ${i.count} db (${Math.round(i.priceRange.min/1e3)}k - ${Math.round(i.priceRange.max/1e3)}k Ft)`).join(`
`))||"",re=s.length>0?s.slice(0,8).map(i=>{var A;return`• ${i.name} | ${(i.salePrice||i.price||0).toLocaleString("hu-HU")} Ft | ${((A=i.category)==null?void 0:A.split(" > ")[0])||"Egyéb"}${i.salePrice&&i.salePrice<i.price?" [AKCIÓ!]":""}`}).join(`
`):"Nem találtam pontos egyezést, de tudok ajánlani hasonlókat!",ae=d.length>0?`Korábban nézett: ${d.map(i=>i.name).join(", ")}`:"",ne=n?`
FELISMERT SZÁNDÉK:
...
- Akciót keres: ${n.isOnSale?"IGEN":"nem"}`:"",ie=p!=null&&p.styleDNA?`Stílus preferencia: ${p.styleDNA}`:"",le=`Te a Marketly bútorwebshop LEGJOBB AI tanácsadója vagy! A világ legokosabb bútorszakértője. Teljes kínálatot ismered.
...
VÁLASZOLJ MOST a fenti szabályok szerint:`;let E={success:!1,text:""};try{E=await be(le,{temperature:.7,maxTokens:500})}catch(i){console.error("AI generateText error:",i)}const G=C();if(E.success&&E.text)k(i=>[...i,{id:G,role:"assistant",content:E.text,timestamp:new Date,products:s.slice(0,8),userQuery:e}]),ke(`${e.slice(0,50)}... kerestél.`);else{const i=s.length>0?s:$("kanapé",4),A=s.length>0?`Találtam ${s.length} terméket! 👇 Nézd meg őket lent, és kattints a részletekért!`:"Jelenleg nem tudok konkrét terméket ajánlani, de böngészd a kategóriákat a menüben!";k(oe=>[...oe,{id:G,role:"assistant",content:A,timestamp:new Date,products:i,userQuery:e,isError:!s.length}])}}catch(l){console.error("Chat hiba:",l);const c=s.length>0?s:$("kanapé",4),d=s.length>0?`Találtam ${s.length} terméket! 👇 Nézd meg őket lent, és kattints a részletekért!`:"Jelenleg nem tudok konkrét terméket ajánlani, de böngészd a kategóriákat a menüben!";k(u=>[...u,{id:C(),role:"assistant",content:d,timestamp:new Date,products:c,userQuery:e,isError:!s.length}])}finally{K(!1)}},W=e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),H())},ee=h.useMemo(()=>{var m;const e=[];V.slice(0,2).forEach(o=>{e.push(o.text)});const a=T(2);if(a.length>0){const o=(m=a[0].category)==null?void 0:m.split(" > ")[0];o&&!e.some(x=>x.includes(o))&&e.push(`${o} 100 ezer alatt`)}const s=ge(2);return s.length>0&&s[0].query&&!e.includes(s[0].query)&&e.push(s[0].query),["Modern kanapé 150 ezer alatt","Skandináv stílusú bútorok","Akciós termékek","Hálószoba berendezés","Étkező bútorok"].forEach(o=>{e.length<5&&!e.includes(o)&&e.push(o)}),e.slice(0,5)},[V]),te=e=>(e||0).toLocaleString("hu-HU")+" Ft";return t.jsxs(t.Fragment,{children:[!j&&t.jsxs("button",{id:"mkt-butorbolt-chat",onClick:()=>S(!0),className:`
            ...
            group
          `,"aria-label":"AI Chat megnyitása",children:[t.jsx(ye,{className:"w-7 h-7"}),t.jsx("span",{className:"absolute -top-1 -right-1 w-6 h-6 bg-green-500 rounded-full flex items-center justify-center text-[10px] font-bold shadow-lg",children:"AI"}),t.jsx("span",{className:"absolute right-full mr-3 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none",children:"AI Tanácsadó"})]}),j&&t.jsxs("div",{className:`
        ...
        `,children:[t.jsxs("div",{className:"bg-gradient-to-r from-primary-500 to-secondary-700 p-4 flex items-center justify-between",children:[t.jsxs("div",{className:"flex items-center gap-3",children:[t.jsx("div",{className:"w-11 h-11 rounded-full bg-white/20 flex items-center justify-center",children:t.jsx(ve,{className:"w-6 h-6 text-white"})}),t.jsxs("div",{children:[t.jsx("h3",{className:"text-white font-bold text-lg",children:"AI Tanácsadó"}),t.jsxs("p",{className:"text-white/80 text-xs flex items-center gap-1.5",children:[t.jsx("span",{className:"w-2 h-2 bg-green-400 rounded-full animate-pulse"}),((B=r==null?void 0:r.total)==null?void 0:B.toLocaleString("hu-HU"))||0," termék ismerete"]})]})]}),t.jsx("button",{onClick:()=>S(!1),className:"text-white/80 hover:text-white transition-colors p-2 hover:bg-white/10 rounded-full","aria-label":"Bezárás",children:t.jsx(we,{className:"w-6 h-6"})})]}),t.jsxs("div",{className:"flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-gray-50 to-white",children:[y.map(e=>t.jsxs("div",{children:[t.jsxs("div",{className:`flex gap-3 ${e.role==="user"?"flex-row-reverse":""}`,children:[t.jsx("div",{className:`
                  ...
                  `,children:e.role==="user"?t.jsx(je,{className:"w-5 h-5 text-white"}):e.isError?t.jsx(Ne,{className:"w-5 h-5 text-white"}):t.jsx(Q,{className:"w-5 h-5 text-white"})}),t.jsxs("div",{className:"max-w-[85%]",children:[t.jsx("div",{className:`
                      rounded-2xl p-3.5
