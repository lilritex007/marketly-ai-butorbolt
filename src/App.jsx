...
const Navbar = ({ activeTab, setActiveTab, wishlistCount, productCount = 0, onScrollToProducts, recentlyViewed = [], categories = [] }) => {
  ...
  return (
    <>
      {/* Animated Announcement Bar - COMPACT on mobile */}
      <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white py-1.5 sm:py-2.5 px-2 sm:px-4 text-center relative overflow-hidden">
        ...
        {/* Animated message */}
        <div className="relative flex items-center justify-center gap-1.5 sm:gap-3">
          <div 
            key={announcementIndex}
            className="flex items-center gap-1.5 sm:gap-2 animate-fade-in"
          >
            <currentAnnouncement.icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" />
            <span className="text-xs sm:text-sm font-medium truncate max-w-[280px] sm:max-w-none">
              {currentAnnouncement.text.split(currentAnnouncement.highlight).map((part, i, arr) => (
                <React.Fragment key={i}>
                  {part}
                  {i < arr.length - 1 && (
                    <span className="font-bold bg-white/20 px-1 py-0.5 rounded mx-0.5 sm:mx-1 text-[11px] sm:text-sm">
      ...
      {/* Returning User Greeting - compact */}
      {isReturningUser && (
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-b border-green-100 py-1.5 px-2 text-center">
      ...
      <nav 
        ...
        `}
      >
        <div className="w-full max-w-[2000px] mx-auto px-2 sm:px-4 lg:px-10 xl:px-16">
          <div className={`flex justify-between items-center transition-all duration-300 ${isScrolled ? 'h-14 sm:h-16 lg:h-18' : 'h-14 sm:h-16 lg:h-22'}`}>
            
            {/* Logo - COMPACT on mobile */}
            <div className="flex items-center cursor-pointer group" onClick={() => setActiveTab('shop')}>
              <div className={`
                relative bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 rounded-lg sm:rounded-xl 
                flex items-center justify-center text-white mr-2 sm:mr-3
                transform group-hover:scale-105 transition-all duration-300 
                shadow-md sm:shadow-lg shadow-indigo-300/50
                ${isScrolled ? 'w-9 h-9 sm:w-11 sm:h-11' : 'w-10 h-10 sm:w-12 sm:h-12'}
              `}>
                <Home className={`${isScrolled ? 'w-4 h-4 sm:w-5 sm:h-5' : 'w-5 h-5 sm:w-6 sm:h-6'}`} />
              ...
              <div className="flex flex-col">
                <div className="flex items-baseline">
                  <span className={`font-black text-gray-900 tracking-tight transition-all ${isScrolled ? 'text-lg sm:text-xl' : 'text-xl sm:text-2xl'}`}>
                    Marketly
                  </span>
                  <span className={`font-black bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent transition-all ${isScrolled ? 'text-lg sm:text-xl' : 'text-xl sm:text-2xl'}`}>
                    .AI
                  </span>
                </div>
                <span className="text-[9px] sm:text-[10px] text-gray-500 font-semibold tracking-widest -mt-0.5 hidden xs:block">
                  BÚTORBOLT
                </span>
              </div>
            </div>
        ...
        {activeTab === 'shop' && (
          <>
            ...
            <section id="products-section" className="container-app section-padding">
                {/* Sticky products header - TIGHTER on mobile */}
                <div className="sticky top-14 sm:top-16 z-40 -mx-2 sm:-mx-4 lg:-mx-8 xl:-mx-10 px-2 sm:px-4 lg:px-8 xl:px-10 py-2 sm:py-3 lg:py-5 mb-2 sm:mb-4 lg:mb-8 bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-sm">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-3 lg:gap-6">
                    {/* Title & Count */}
                    <div className="flex items-baseline gap-2 sm:gap-3">
                      <h2 className="text-lg sm:text-xl lg:text-3xl xl:text-4xl font-bold text-gray-900">Termékek</h2>
                      {!isLoadingUnas && (
                        <span className="text-xs sm:text-sm lg:text-lg xl:text-xl text-gray-500">
