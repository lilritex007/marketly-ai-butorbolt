    ...
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      ...
      <div className="flex overflow-x-auto border-b border-gray-200 scrollbar-hide">
        {tabs.map((tab) => (
          <button
            ...
            className={`
              flex items-center gap-2 px-4 sm:px-6 py-4 font-medium text-sm whitespace-nowrap border-b-2 transition-all
              ${activeTab === tab.id 
      ...
      <div className="p-5 sm:p-6">
        ...
        {activeTab === 'specs' && (
          <div>
            {specs.length > 0 ? (
              ...
              <p className="text-gray-500 text-center py-8">Nincs elérhető műszaki adat.</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'reviews' && (
          <div>
            {/* Summary */}
            <div className="flex items-center gap-6 p-4 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl mb-6">
              <div className="text-center">
                ...
                <p className="text-xs text-gray-500 mt-1">{reviews.length} vélemény</p>
              </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="font-medium text-gray-900 text-sm flex items-center gap-2">
                          {review.author}
                          {review.verified && (
                            <span className="px-1.5 py-0.5 bg-green-100 text-green-700 text-[10px] font-bold rounded">✓ Ellenőrzött</span>
                          )}
                        </p>
                        <p className="text-xs text-gray-400">{new Date(review.date).toLocaleDateString('hu-HU')}</p>
                      </div>
                    </div>
                    <div className="flex">
                      {[1,2,3,4,5].map(star => (
            ...
            {/* Write review button */}
            <button className="w-full mt-4 py-3 border-2 border-dashed border-gray-200 text-gray-500 font-medium rounded-xl hover:border-indigo-300 hover:text-indigo-600 transition-colors">
              + Vélemény írása
            </button>
          </div>
        )}
        ...
        {activeTab === 'faq' && (
          <div className="space-y-3">
            ...
            {/* Ask question */}
            <button className="w-full mt-2 py-3 border-2 border-dashed border-gray-200 text-gray-500 font-medium rounded-xl hover:border-indigo-300 hover:text-indigo-600 transition-colors flex items-center justify-center gap-2">
        ...
        {activeTab === 'shipping' && (
          <div className="space-y-4">
            <div className="flex items-start gap-4 p-4 bg-green-50 rounded-xl">
              ...
              <div>
                <p className="font-bold text-green-800">Ingyenes szállítás 50.000 Ft felett</p>
                <p className="text-green-700 text-sm">Standard szállítás: 2-5 munkanap</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-4 p-4 bg-blue-50 rounded-xl">
              ...
              <div>
                <p className="font-bold text-blue-800">2 év garancia</p>
                <p className="text-blue-700 text-sm">Teljes körű gyártói garancia minden termékre</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              <div className="p-3 bg-gray-50 rounded-lg text-center">
                ...
                <p className="text-xs text-gray-500">munkanap</p>
              </div>
              <div className="p-3 bg-gray-50 rounded-lg text-center">
                <p className="text-2xl font-bold text-gray-900">14</p>
                <p className="text-xs text-gray-500">nap elállás</p>
              </div>
            </div>
          </div>
        )}
